...
exports.getAllUsers = async (req, res) => {
  try {
    const users = await User.find({ societyId: req.societyId, isActive: true })
      .select('-password')
      .sort({ name: 1 });
...
exports.getProfessionals = async (req, res) => {
  try {
    ...
    const query = { societyId: req.societyId, isServiceProvider: true, isActive: true };
...
exports.updateAvailability = async (req, res) => {
  try {
    ...
    const user = await User.findOneAndUpdate(
      { _id: req.user._id, societyId: req.societyId },
...
exports.getUserById = async (req, res) => {
  try {
    const user = await User.findOne({ _id: req.params.id, societyId: req.societyId }).select('-password');
...
exports.updateUserRole = async (req, res) => {
  try {
    ...
    req.emitToSociety('user_role_updated', user);
    res.json({ success: true, user });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.savePushToken = async (req, res) => {
  try {
    ...
    await User.updateOne({ _id: req.user._id, societyId: req.societyId }, { pushToken });
...
exports.getStats = async (req, res) => {
  try {
    ...
    const allUsers = await User.find({ societyId: req.societyId, isActive: true }).select('houseNo');
